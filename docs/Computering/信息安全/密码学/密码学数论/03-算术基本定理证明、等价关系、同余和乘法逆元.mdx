...
> 定理7-1：设$p$是素数，$a,b\in\mathbb{Z}$，则$p|ab\rightarrow p|a$或$p|b$。
> 推论：设$p$是素数，$a_1,\ldots,a_k\in\mathbb{Z}$，则$p|a_1\ldots a_k\rightarrow p|a_i$，$i\in\{1,k\}$。
...
> 对称性：对于所有$\triangle A,\triangle B\in T$，都有$\triangle A\cong\triangle B\rightarrow\triangle B\cong\triangle A$。
> 传递性：对于所有$\triangle A,\triangle B,\triangle C\in T$，都有$\triangle A\cong\triangle B$，$\triangle B\cong\triangle C\rightarrow\triangle A\cong\triangle C$。
...
> 定义（乘法逆元）：设$a\in\mathbb{Z}$，$n\in\mathbb{N}$，如果$az\equiv1\pmod n$，称$z$是模$n$下$a$的乘法逆元，记作$a^{-1}=z$。
...
2. $a$在模$n$内的乘法逆元$a^{-1}$（$1\le a^{-1}\<n$）是唯一的。
3. 乘法逆元存在的条件：$\gcd(a,n)=1\longleftrightarrow$模$n$下，$a$有乘法逆元。

> 设$\gcd(a,n)=d\ (d>1)$。
> 假设$a^{-1}=b$，则$ab\equiv1\pmod n$，有$ab=qn+1$，
> 即$ab-qn=1$。
> 因为$d\mid a$，$d\mid n$，
> 所以$d\mid1$，矛盾，假设不成立。
