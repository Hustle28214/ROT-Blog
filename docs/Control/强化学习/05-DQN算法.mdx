...
环境定义与参数可查阅 [Gymnasium CartPole-v1 官方文档](https://gymnasium.farama.org/environments/classic_control/cart_pole/)。

它的状态值是连续的，动作值是离散的。

这是一个典型的倒立摆小车环境。智能体通过左右移动小车使杆保持竖直；杆的倾角或小车位置超出阈值时回合终止，达到 `CartPole-v1` 的 500 步时间限制时回合截断。状态是 4 维连续向量，动作空间包含 2 个离散动作。
...
从而将 Q-learning 扩展为神经网络形式，也就是深度 Q 网络（Deep Q-Network，DQN）。

DQN 是离策略（off-policy）算法，因此收集数据时可以使用 $\epsilon$-贪婪策略平衡探索与利用，再用这些经验训练目标策略。
...
class ReplayBuffer:
    ...
    def add(self, state: np.ndarray, action: int, reward: float,
            next_state: np.ndarray, terminated: bool):
        """添加经验到缓冲区"""
    ...
    def sample(self) -> Tuple:
        ...
        terminateds = torch.from_numpy(
            np.vstack([e[4] for e in experiences]).astype(np.uint8)
        ).float()
...
class DQNAgent:
    ...
    def learn(self, experiences: Tuple):
        ...
        q_targets = rewards + self.gamma * q_targets_next * (1 - terminateds)
...
def train_dqn(env_name: str = "CartPole-v1", n_episodes: int = 1000,
              max_t: int = 500, eps_start: float = 1.0,
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        # CartPole-v1 的参考成功阈值为最近 100 回合平均 475 分
...
if __name__ == "__main__":
    ...
    print("开始训练DQN智能体...")
    scores = train_dqn(n_episodes=1000, seed=seed)
    
    print("绘制训练曲线...")
    plot_scores(scores)
    
    print("测试训练好的智能体...")
    test_agent(seed=seed)
```

训练曲线和求解回合数会随依赖版本、设备及随机轨迹变化。下一篇将继续介绍 [Double DQN 与 Dueling DQN](./06-DQN改进算法.mdx)，分别改进目标值估计和网络结构。
