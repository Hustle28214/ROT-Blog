...
### 2.1 在 FrozenLake-v1 中比较两种算法

Gymnasium 的环境交互接口使用 `reset()` 返回 `(observation, info)`，`step()` 返回五元组；动态规划不需要采样，而是直接读取解封装环境公开的转移模型 `P`。下面复用本章的 `policy_iteration` 与 `value_iteration`，并用一个轻量适配器统一状态数、动作数和转移模型的属性名：
...
model = env.unwrapped
dp_env = SimpleNamespace(
    nS=env.observation_space.n,
    nA=env.action_space.n,
    P=model.P,
)

pi_policy, pi_values = policy_iteration(dp_env, gamma=0.9, theta=1e-8)
vi_policy, vi_values = value_iteration(dp_env, gamma=0.9, theta=1e-8)

action_symbols = np.array(["←", "↓", "→", "↑"])
desc = np.asarray(model.desc, dtype="S1")

def print_frozenlake_policy(name, policy, values):
    symbols = action_symbols[np.argmax(policy, axis=1)].reshape(desc.shape)
    display = symbols.astype("<U1")
    display[desc == b"H"] = "H"
    display[desc == b"G"] = "G"
    print(f"\n{name}")
    print(display)
    print(np.round(values.reshape(desc.shape), 3))

print_frozenlake_policy("策略迭代", pi_policy, pi_values)
print_frozenlake_policy("价值迭代", vi_policy, vi_values)
print("两种方法的价值函数是否一致：", np.allclose(pi_values, vi_values, atol=1e-6))
