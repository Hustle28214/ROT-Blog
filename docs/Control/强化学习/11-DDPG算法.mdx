...
之前的章节介绍了基于策略梯度的 REINFORCE、Actor-Critic、TRPO，以及上一章的 [PPO 算法](./10-PPO算法.mdx)。这类算法有一个共同的特点：它们都是在线策略算法，这意味着它们的样本效率比较低。
        ...
        with tqdm(total=int(num_episodes/10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes/10)):
                ...
                while not episode_done:
                    ...
                    replay_buffer.add(state, action, float(reward), next_state,
                                      terminated)
                    state = next_state
