...
前面的 [DQN](./05-DQN算法.mdx) 及其[改进算法](./06-DQN改进算法.mdx) 学习动作价值函数，再通过 $\epsilon$-贪婪规则隐式地产生策略。策略梯度方法则直接学习显式策略 $\pi_\theta(a|s)$：输入状态，输出动作概率分布，并以期望回报为目标优化参数。本篇先介绍策略梯度定理，再用 REINFORCE 给出蒙特卡洛实现。
## 1. 策略梯度
...
由于期望 $\mathbb{E}$ 的下标是 $\pi_\theta$，这里的估计必须使用当前策略采样的数据，因此 REINFORCE 是同策略（on-policy）算法。这里的“同策略”描述行为策略与目标策略的关系，不等同于“在线学习”。
...
# 测试训练好的策略
def test_policy(env, agent, num_episodes=10):
    print("\n测试训练好的策略...")
    test_scores = []
...
def main():
    ...
    test_scores = test_policy(test_env, agent, num_episodes=5)
    test_env.close()
...
## 3. 从 REINFORCE 到 Actor-Critic

REINFORCE 在回合结束后，用完整蒙特卡洛回报 $G_t$ 更新策略。它无须学习价值函数，估计通常无偏，但方差较大且更新有延迟。一个自然的改进是学习状态价值函数 $V_\phi(s)$，用一步 TD 误差

$$
\delta_t=r_t+\gamma(1-d_t)V_\phi(s_{t+1})-V_\phi(s_t)
$$

代替 $G_t$ 作为策略更新信号。这样便得到 [Actor-Critic 算法](./08-Actor-Critic算法.mdx)：Actor 表示策略，Critic 估计价值。它通过引入一定的自举偏差换取更低方差和逐步更新能力。
