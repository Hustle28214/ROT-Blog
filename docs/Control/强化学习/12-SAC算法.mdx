...
上一章的 [DDPG 算法](./11-DDPG算法.mdx) 虽然样本效率较高，但训练过程就像在钢丝上行走，稍有不慎就会失去平衡。SAC 算法的出现改变了这一局面，它巧妙地将探索与利用相结合，在保持高效率的同时显著提升了训练的稳定性。
        ...
        with tqdm(total=int(num_episodes/10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes/10)):
                ...
                while not episode_done:
                    ...
                    # 回放池中的 done 仅屏蔽真正终止状态的 bootstrap
...
if __name__ == "__main__":
    ...
    return_list = train_off_policy_agent(
        env, agent, num_episodes, replay_buffer, minimal_size, batch_size, seed=0
    )
...
SAC 仍然完全依赖真实环境采样。下一章将转向第 13 章 MBPO：先学习环境动力学模型，再用模型生成短轨迹辅助策略更新，从“无模型、离线策略”自然过渡到基于模型的强化学习。
