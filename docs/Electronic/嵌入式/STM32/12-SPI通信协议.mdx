...
:::tip
这个笔记是在江科大的课程那里做的，当时还很陌生，现在回来看，发现SPI通信协议其实很简单。

其实就是一个占IO的高速通信协议，你要找不同的设备，既不像uart那种要用id，也不像i2c那种要用地址。每个设备都有一个唯一的片选引脚，全部连到主机的IO上，你要找不同的设备，就一个个拉低（拉高）。

还有一种连接方式叫菊花链，所有从设备共享同一条片选线（CS）、时钟线和数据线。在硬件上，它们被串联成一条链：主设备的MOSI连接到第一个从设备的MOSI，第一个从设备的MISO（或数据输出DOUT）连接到第二个从设备的MOSI，依此类推。

好处是如果你做通信设备不多的项目，SPI通信协议是一个很好的选择。坏处就是IO资源占用多。你设计一个功能复杂传感器很多的项目，那你就要斟酌一下引脚分配。实际上，很多高速通信的器件同时支持I2C和SPI，看需求取舍，要么快而器件少挂SPI，要么慢而器件多挂I2C，就这么简单。
:::
